...
//	  mod, chan, proj, xy, z,   dt, ecoef 
	...
	PMT(1,    3,    0,  5, 3, 44.0,  8.29);
	PMT(1,    4,    0,  5, 2, 51.4,  9.85);
	PMT(1,    5,    0,  5, 1, 45.1,  9.04);

	PMT(1,    6,    0,  4, 5, 45.5,  8.78);
	PMT(1,    7,    0,  4, 4, 47.9,  8.66);
	PMT(1,    8,    0,  4, 3, 45.9,  7.96);
	PMT(1,    9,    0,  4, 2, 50.3,  9.28);
	PMT(1,   10,    0,  4, 1, 52.0,  8.90);

	PMT(1,   11,    0,  3, 5, 43.3,  7.62);
	PMT(1,   12,    0,  3, 4, 50.3,  7.71);
	PMT(1,   13,    0,  3, 3, 50.0,  8.03);
	PMT(1,   14,    0,  3, 2, 55.1, 10.74);
	PMT(1,   15,    0,  3, 1, 50.7,  8.84);

	PMT(1,   51,    0,  2, 5, 45.2,  7.73);
	PMT(1,   17,    0,  2, 4, 46.2,  8.03);
	PMT(1,   18,    0,  2, 3, 49.8,  7.37);
	PMT(1,   19,    0,  2, 2, 48.0, 10.33);
	PMT(1,   20,    0,  2, 1, 49.2,  8.87);

	PMT(1,   21,    0,  1, 5, 44.9,  8.60);
	PMT(1,   22,    0,  1, 4, 45.5,  7.27);
	PMT(1,   23,    0,  1, 3, 47.9,  7.60);
	PMT(1,   24,    0,  1, 2, 55.7,  9.16);
	PMT(1,   25,    0,  1, 1, 53.7, 11.93);

	PMT(1,   26,    1,  1, 5, 54.5,  9.47);
	PMT(1,   27,    1,  1, 4, 53.3,  8.61);
	PMT(1,   28,    1,  1, 3, 49.2,  8.72);
	PMT(1,   29,    1,  1, 2, 50.5,  8.12);
	PMT(1,   30,    1,  1, 1, 52.1,  8.52);

	PMT(1,   31,    1,  2, 5, 46.9,  7.85);
	PMT(1,   32,    1,  2, 4, 50.0,  7.79);
	PMT(1,   33,    1,  2, 3, 52.8,  8.15);
	PMT(1,   34,    1,  2, 2, 46.1,  8.23);
	PMT(1,   35,    1,  2, 1, 48.7,  8.52);

	PMT(1,   36,    1,  3, 5, 46.9,  8.59);
	PMT(1,   37,    1,  3, 4, 47.2,  9.67);
	PMT(1,   38,    1,  3, 3, 46.7,  9.08);
	PMT(1,   39,    1,  3, 2, 49.7,  8.93);
	PMT(1,   40,    1,  3, 1, 48.4,  8.17);

	PMT(1,   41,    1,  4, 5, 48.6,  9.19);
	PMT(1,   42,    1,  4, 4, 52.5,  9.90);
	PMT(1,   43,    1,  4, 3, 47.2,  8.03);
	PMT(1,   44,    1,  4, 2, 46.7,  8.82);
	PMT(1,   45,    1,  4, 1, 45.7,  8.19);

	PMT(1,   46,    1,  5, 5, 52.8,  8.39);
	PMT(1,   47,    1,  5, 4, 48.7,  8.98);
	PMT(1,   48,    1,  5, 3, 48.0,  8.90);
	PMT(1,   49,    1,  5, 2, 51.4,  7.08);
	PMT(1,   50,    1,  5, 1, 53.0,  7.95);
//	   mod, chan,   dt, ecoef 
	VETO(3,    1, 45.7,  4.0);
	VETO(3,    2, 50.2,  4.0);
	VETO(3,    3, 42.9,  4.0);
